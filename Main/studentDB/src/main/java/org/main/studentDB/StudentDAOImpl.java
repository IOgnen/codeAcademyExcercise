...
public class StudentDAOImpl implements StudentDAO {
	...
	public void createStudentTable() {
		...
		try {
			...
			if (!status) {
				...
				System.out.println("pojma nemas");
			}
	...
	public void createProfesorTable() {
		...
		try {
			...
			String query = "CREATE TABLE public.profesor("
						+ "prof_id SERIAL,"
						+ "name VARCHAR(20),"
						+ "email VARCHAR(30),"
						+ "PRIMARY KEY(prof_id)"
	...
	public void createSubjectTable() {
		...
		try {
			...
			String query = "CREATE TABLE public.subject("
						+ "subject_id SERIAL,"
						+ "name VARCHAR(20),"
						+ "prof_id INTEGER,"
	...
	public void insertProfesor(List<Student> students) {
		...
		try {
			...
			String query = "INSERT INTO public.profesor("
					+ "	name, email)"
					+ "	VALUES (?, ?);";
	...
	public void createStudentSubjectTable() {
		...
		try {
			...
			String query = "CREATE TABLE public.student_subject("
						+ "index INTEGER,"
						+ "subject_id INTEGER,"
						+ "ocenka INTEGER,"
						+ "FOREIGN KEY(index) REFERENCES public.student(index),"
	...
	public void searchPolozheniMakedonski(SubjectNames s, Integer ocenka) {
		...
		try {
			...
			String query = "SELECT * FROM public.student "
					+ "JOIN public.student_subject "
					+ "ON public.student.index = public.student_subject.index "
					+ "WHERE subject_id="+subjectId+" AND ocenka>"+ocenka+";";
			...
			while(rs.next()) {
				...
				Integer o = rs.getInt(7);
			
				System.out.println("Index= "+ index + " name= " + name + " surname= "+surname+ " city= "+city+" subject = "+s.toString()+" ocenka= "+o);
	...
	public void searchProfesor(Integer index, SubjectNames s) {
		...
		try {
			...
			String query = "SELECT * FROM public.student_subject JOIN subject ON student_subject.subject_id=subject.subject_id JOIN profesor ON subject.prof_id=profesor.prof_id WHERE index="+index+" AND subject.name='"+s.toString()+"';";
